...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/answer/manage?method=delete&*','post')" @click="deleteAnswer($event,undefined)">批量删除</el-button>
        </div>
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" stripe empty-text="没有内容">
                ...
                <el-table-column prop="content" label="答案内容" align="center"></el-table-column>
                ...
                <el-table-column prop="postTime" label="回答时间" align="center" width="160"></el-table-column>
                <el-table-column label="操作" align="center" width="150">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/question/manage?method=view*','get')" @click="store.setCacheNumber();$router.push({path: '/admin/control/question/manage/view', query:{ questionId : scope.row.questionId,answerId:scope.row.id}})">查看</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/answer/manage?method=delete&*','post')" @click="deleteAnswer($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    //查询答案列表
    const queryAnswerList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/question/allAuditAnswer',
            method: 'get',
            params: {
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/question/allAuditAnswer', 
            query:{ page : page}
        });
    }
    ...
    //删除答案
    const deleteAnswer = (event:any,row:any) => {
        //强制失去焦点
        ...
        .then(() => {
            ...
            for(let i=0; i<state.multipleSelection.length; i++){
                ...
                formData.append('answerId', rowData.id);
            }
