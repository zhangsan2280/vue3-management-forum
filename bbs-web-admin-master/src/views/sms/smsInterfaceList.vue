...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/smsInterface/manage?method=add*','get')" @click="$router.push({path: '/admin/control/smsInterface/manage/add'});">添加短信接口</el-button>
        </div>
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                ...
                <el-table-column prop="uses" label="是否启用" align="center" min-width="100">
                    <template #default="scope">	
                        <el-switch v-model="state.enable[scope.$index]" :disabled="!verifyPermissionMenu('/control/smsInterface/manage?method=enableInterface*','post')" @change="handleSmsChange(scope.row,scope.$index)"></el-switch>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/smsInterface/manage?method=edit*','get')" @click="$router.push({path: '/admin/control/smsInterface/manage/edit', query:{ smsInterfaceId : scope.row.id, page:state.currentpage}})">修改</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/smsInterface/manage?method=delete*','post')" @click="deleteSmsInterface($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    //处理选择模板
    const handleSmsChange = (row:any,index:number) => {
        if(row.enable && !state.enable[index]){//如果不选中
            state.enable[index] = true;
            return;
        }
    ...
    //查询短信接口列表
    const querySmsInterfaceList = () => {
        //清空内容
        state.tableData = []; 
		state.enable = [];

        proxy?.$axios({
            url: '/control/smsInterface/list',
            method: 'get',
            params: {
    ...
    //删除短信接口
    const deleteSmsInterface = (event:any,row:any) => {
        //强制失去焦点
