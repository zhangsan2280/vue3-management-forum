        ...
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                ...
                <el-table-column label="操作" align="center" width="130">
                    <template #default="scope">
                        <el-button-group>
                            <el-button type="primary" :disabled="!verifyPermissionMenu('/control/systemNotify/manage?method=reductionSubscriptionSystemNotify&*','post')" v-if="scope.row.status >100" @click="reductionSubscriptionSystemNotify($event,scope.row)">还原</el-button>
                        </el-button-group>
                    </template>
...
<script lang="ts" setup>
    ...
    //查询系统通知
    const querySubscriptionSystemNotifyList = () => {
        //清空内容
        state.tableData = []; 
   ...
   //还原系统通知
   const reductionSubscriptionSystemNotify = (event:any,row:any) => {
        //强制失去焦点
        ...
        ElMessageBox.confirm('此操作将还原该项状态, 是否继续?',{
            // type: 'warning',
            cancelButtonText: "取消",
                ...
                if(response){
                    ...
                    if(result){
				    	...
				    	if(returnValue.code === 200){//成功
				    		...
				    		querySubscriptionSystemNotifyList();
				    	}else if(returnValue.code === 500){//错误
