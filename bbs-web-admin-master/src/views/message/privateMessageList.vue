    ...
    <div class="main">
        ...
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" @selection-change="handleSelectionChange" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                ...
                <el-table-column label="对方用户" align="center" width="240">
                    <template #default="scope">
                        <el-popover effect="light" trigger="hover" placement="top">
                            <template #default>
                                <p v-if="scope.row.friendNickname != null && scope.row.friendNickname != ''">呢称: {{scope.row.friendNickname}}</p>
                                <p>账号: {{scope.row.friendAccount}}</p>
                            </template>
                            <template #reference>
                                <div class="avatar-wrapper" >
                                    <div class="avatar-badge" v-if="scope.row.friendAvatarName == null || scope.row.friendAvatarName == ''">
                                        <el-avatar :size="48" :icon="UserFilled"></el-avatar>
                                    </div>
                                    <div class="avatar-badge" v-if="scope.row.friendAvatarName != null && scope.row.friendAvatarName != ''">
                                        <el-avatar :size="48" :src="scope.row.friendAvatarPath+'100x100/'+scope.row.friendAvatarName"></el-avatar>
                                    </div>
                                    
                                    <div class="avatar-text">{{scope.row.friendAccount}}</div>
                                </div>
                            </template>
                        </el-popover>
                    </template>
                </el-table-column>
                <el-table-column prop="messageContent" label="最新私信内容" align="center"></el-table-column>
                <el-table-column prop="sendTime" label="发送时间" align="center" width="160"></el-table-column>
                <el-table-column label="操作" align="center" width="150">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/privateMessage/manage?method=privateMessageChatList&*','get')" @click="$router.push({path: '/admin/control/privateMessage/manage/privateMessageChatList', query:{ friendUserId : scope.row.friendUserId, privateMessagePage:state.currentpage, id : $route.query.id,userName : encodeURIComponent(state.userName),beforeUrl:($route.query.beforeUrl != undefined ? $route.query.beforeUrl:'')}})">对话</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/privateMessage/manage?method=deletePrivateMessageChat&*','post')" @click="deletePrivateMessage($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/privateMessage/manage/privateMessageList', 
            query:{ 
                id:state.id,
   ...
   //删除私信
   const deletePrivateMessage = (event:any,row:any) => {
        //强制失去焦点
        ...
        ElMessageBox.confirm('此操作将删除该私信, 是否继续?',{
            // type: 'warning',
            cancelButtonText: "取消",
        ...
        .then(() => {
            ...
            for(let i=0; i<state.multipleSelection.length; i++){
                ...
                formData.append('friendUserId', rowData.friendUserId);
            }

            proxy?.$axios({
                url: '/control/privateMessage/manage?method=deletePrivateMessageChat',
                method: 'post',
                data: formData
            })
            .then((response: AxiosResponse) => {
