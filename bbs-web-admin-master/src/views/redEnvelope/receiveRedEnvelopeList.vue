    ...
    <div class="main">
        ...
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                <el-table-column label="发红包用户" align="center" >
                    <template #default="scope">
                        <el-popover effect="light" trigger="hover" placement="top">
                            <template #default>
                                <p v-if="scope.row.giveNickname != null && scope.row.giveNickname != ''">呢称: {{scope.row.giveNickname}}</p>
                                <p>账号: {{scope.row.giveAccount}}</p>
                            </template>
                            <template #reference>
                                <div class="avatar-wrapper" >
                                    <div class="avatar-badge" v-if="scope.row.giveAvatarName == null || scope.row.giveAvatarName == ''">
                                        <el-avatar :size="48" :icon="UserFilled"></el-avatar>
                                    </div>
                                    <div class="avatar-badge" v-if="scope.row.giveAvatarName != null && scope.row.giveAvatarName != ''">
                                        <el-avatar :size="48" :src="scope.row.giveAvatarPath+'100x100/'+scope.row.giveAvatarName"></el-avatar>
                                    </div>
                                    
                                    <div class="avatar-text">{{scope.row.giveAccount}}</div>
                                </div>
                            </template>
                        </el-popover>
                    </template>
                ...
                <el-table-column prop="receiveTime" label="收取时间" align="center" width="170"></el-table-column>
            </el-table>
...
<script lang="ts" setup>
    ...
    //查询收红包列表
    const queryReceiveRedEnvelopeList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/redEnvelope/receiveRedEnvelope/list',
            method: 'get',
            params: {
                id :state.id,
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/redEnvelope/receiveRedEnvelope/list', 
            query:{ 
                id:state.id,
