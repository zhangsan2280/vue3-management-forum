            ...
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                <el-table-column label="类型" align="center" width="170">
                    <template #default="scope">
                        <span v-if="scope.row.type == 10">个人定向红包</span>
                        <span v-if="scope.row.type == 20">公共随机红包</span>
                        <span v-if="scope.row.type == 30">公共定额红包</span>
                    </template>
                </el-table-column>
                <el-table-column label="总金额" align="center" >
                    <template #default="scope">
                        {{scope.row.totalAmount}}
                        <span v-if="scope.row.refundAmount >0" style="color: red;">中止领取红包后返还金额￥{{scope.row.refundAmount}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="giveQuantity" label="发放数量" align="center" width="100"></el-table-column>
                <el-table-column label="已领取数量" align="center" width="100">
                    <template #default="scope">
                ...
                <el-table-column label="话题" align="center" >
                    <template #default="scope">
                        <el-link href="javascript:void(0);" @click="store.setCacheNumber(); $router.push({path: '/admin/control/topic/manage/view', query:{ topicId : scope.row.bindTopicId,commentId:''}})">{{scope.row.bindTopicTitle}}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="giveTime" label="时间" align="center" width="170"></el-table-column>
                <el-table-column label="操作" align="center" width="100">
                    <template #default="scope">
                        <el-button-group>
                            <el-button type="primary" :disabled="!verifyPermissionMenu('/control/redEnvelope/redEnvelopeAmountDistribution/list*','get')" @click="$router.push({path: '/admin/control/redEnvelope/redEnvelopeAmountDistribution/list', query:{ giveRedEnvelopeId : scope.row.id, giveRedEnvelopePage:state.currentpage, id : $route.query.id,userName : encodeURIComponent(state.userName),beforeUrl:($route.query.beforeUrl != undefined ? $route.query.beforeUrl:'')}})">金额分配</el-button>
                        </el-button-group>
                    </template>
                </el-table-column>
            </el-table>
...
<script lang="ts" setup>
    ...
    //查询发红包列表
    const queryGiveRedEnvelopeList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/redEnvelope/giveRedEnvelope/list',
            method: 'get',
            params: {
                id :state.id,
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/redEnvelope/giveRedEnvelope/list', 
            query:{ 
                id:state.id,
