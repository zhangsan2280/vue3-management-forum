...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/links/manage?method=add*','get')" @click="$router.push({path: '/admin/control/links/manage/add'});">添加友情链接</el-button>
        </div>
                ...
                <el-table-column label="操作" align="center" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/links/manage?method=edit*','get')" @click="$router.push({path: '/admin/control/links/manage/edit', query:{ linksId : scope.row.id}})">修改</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/links/manage?method=delete*','post')" @click="deleteLinks($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
                </el-table-column>
...
<script lang="ts" setup>
    ...
    //查询友情链接列表
    const queryLinksList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/links/list',
            method: 'get',
            params: {
            },
    ...
    //删除友情链接
    const deleteLinks = (event:any,row:any) => {
        //强制失去焦点
