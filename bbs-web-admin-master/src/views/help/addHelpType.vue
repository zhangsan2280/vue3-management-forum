    ...
    <div class="main">
        ...
        <div class="data-form label-width-blank" >
            <el-form label-width="auto"  @submit.native.prevent>
                <el-form-item label="父类名称" v-if="state.parentHelpType != undefined && state.parentHelpType != null && Object.keys(state.parentHelpType).length>0">
                    {{state.parentHelpType.name}}
                    <div class="form-help" v-if="state.parentHelpType.childNodeNumber == 0">添加本分类后，上级分类帮助会自动转移到本分类</div>
                </el-form-item>
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "parentHelpType"){
			    				state.parentHelpType = mapData[key];
