        ...
        <div class="data-form label-width-blank" >
            <el-form label-width="auto"  @submit.native.prevent>
                <el-form-item label="用户类型" >
                    <span v-if="state.type == 10">本地账号密码用户</span>
                    <span v-if="state.type == 20">手机用户</span>
                    <span v-if="state.type == 30">邮箱用户</span>
                    <span v-if="state.type == 40">微信用户</span>
                    <span v-if="state.type == 80">其他用户</span>
                </el-form-item>
...
<script lang="ts" setup>
    ...
    import { SourceUrl, UserCustom, UserRole } from '@/types';
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "userRoleList"){
		    					...
		    					state.issue = user.issue;
		    					state.realNameAuthentication = user.realNameAuthentication;
		    					state.state = user.state;
		    					state.allowUserDynamic = user.allowUserDynamic;
		    					state.remarks = user.remarks;
    ...
    onMounted(() => {
		...
		//上级路由解码
		if(router.currentRoute.value.query.editUser_beforeUrl != undefined && router.currentRoute.value.query.editUser_beforeUrl != ''){
			let parsedWordArray = enc.Base64url.parse(router.currentRoute.value.query.editUser_beforeUrl as string);
