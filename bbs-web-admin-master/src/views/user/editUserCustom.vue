        ...
        <div class="data-form label-width-blank" >
            <el-form label-width="auto"  @submit.native.prevent>
                ...
                <el-form-item label="选择类型" :required="true" :error="error.chooseType" >
                    <el-select v-model="state.chooseType"  no-match-text="还没有类型" placeholder="选择类型" disabled>
...
<script lang="ts" setup>
    ...
    //查询用户自定义注册功能项
    const queryUserCustom = () => {
        proxy?.$axios({
            url: '/control/userCustom/manage',
            method: 'get',
            params: {
                method : 'edit',
			    id: state.id,
			    		...
			    		for(let key in mapData){
			    			if(key == "userCustom"){
			    				...
			    				if(userCustom.name != null){
			    					state.name = userCustom.name;
			    				}
			    				if(userCustom.required != null){
									state.required = userCustom.required;
								}
								if(userCustom.search != null){
									state.search  = userCustom.search;
								}
								if(userCustom.visible != null){
									state.visible = userCustom.visible;
								}
								if(userCustom.sort != null){
									state.sort  = userCustom.sort;
								}
								if(userCustom.tip != null){
									state.tip = userCustom.tip;
								}
								if(userCustom.chooseType != null){
									state.chooseType = userCustom.chooseType;
								}
								if(userCustom.size != null){
									state.size = userCustom.size;
								}
								if(userCustom.maxlength != null){
									state.maxlength = userCustom.maxlength;
								}
								if(userCustom.fieldFilter != null){
									state.fieldFilter = userCustom.fieldFilter;
								}
								if(userCustom.regular != null){
									state.regular = userCustom.regular;
								}
								if(userCustom.multiple != null){
									state.multiple  = userCustom.multiple;
								}
								if(userCustom.selete_size != null){
									state.selete_size = userCustom.selete_size;
								}
								if(userCustom.rows != null){
									state.rows = userCustom.rows;
								}
								if(userCustom.cols != null){
									state.cols  = userCustom.cols;
								}
