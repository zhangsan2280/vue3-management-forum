...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/onlinePaymentInterface/manage?method=add*','get')" @click="$router.push({path: '/admin/control/onlinePaymentInterface/manage/add'});">添加在线支付接口</el-button>
        </div>
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                <el-table-column prop="name" label="支付接口名称" align="center" ></el-table-column>
                <el-table-column label="接口产品" align="center" >
                    <template #default="scope">
                        <span v-if="scope.row.interfaceProduct == 1">支付宝即时到账</span>
                        <span v-if="scope.row.interfaceProduct == 4">支付宝手机网站(alipay.trade.wap.pay)</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/onlinePaymentInterface/manage?method=edit&*','get')" @click="$router.push({path: '/admin/control/onlinePaymentInterface/manage/edit', query:{ onlinePaymentInterfaceId : scope.row.id, page:state.currentpage}})">修改</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/onlinePaymentInterface/manage?method=delete*','post')" @click="deleteOnlinePaymentInterface($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    //查询在线支付接口列表
    const queryOnlinePaymentInterfaceList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/onlinePaymentInterface/list',
            method: 'get',
            params: {
    ...
    //删除在线支付接口
    const deleteOnlinePaymentInterface = (event:any,row:any) => {
        //强制失去焦点
        ...
        .then(() => {
            ...
            formData.append('onlinePaymentInterfaceId', row.id);

            proxy?.$axios({
                url: '/control/onlinePaymentInterface/manage?method=delete',
                method: 'post',
                data: formData
            })
            .then((response: AxiosResponse) => {
                if(response){
                    ...
                    if(result){
				    	...
				    	if(returnValue.code === 200){//成功
				    		...
				    		queryOnlinePaymentInterfaceList();
				    	}else if(returnValue.code === 500){//错误
