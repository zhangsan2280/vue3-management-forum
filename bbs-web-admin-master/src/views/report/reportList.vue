...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="state.visible == 'false'" @click="$router.push({path: '/admin/control/report/list', query:{ visible : 'true'}})">返回</el-button>
            <el-button type="primary" plain v-if="state.visible == 'false' && verifyPermissionMenu('/control/report/manage?method=reduction*','post')" @click="reductionReport($event)">还原</el-button>
            <el-button type="primary" plain v-if="state.visible == 'true'" @click="$router.push({path: '/admin/control/report/list', query:{ visible : 'false'}})">回收站</el-button>
                ...
                <el-table-column label="操作" align="center" fixed="right" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" :disabled="!verifyPermissionMenu('/control/report/manage?method=reportHandle*','post')" v-if="scope.row.status==10" @click="processUI(scope.row)">处理</el-button>
...
<script lang="ts" setup>
    ...
    const state = reactive({
        ...
        visible :'true',//是否显示 true:未删除的举报 false:已删除的举报
        multipleSelection: [],
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/report/list', 
            query:{ visible : state.visible, ip_text : String(state.ip_text), ipAddress_text : String(state.ipAddress_text),page : page}
        });
    }
