...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="state.visible == 'false'" @click="$router.push({path: '/admin/control/topic/list', query:{ visible : 'true'}})">返回</el-button>
            <el-button type="primary" plain v-if="state.visible == 'false' && verifyPermissionMenu('/control/topic/manage?method=reduction*','post')" @click="reductionTopic($event)">还原</el-button>
            <el-button type="primary" plain v-if="state.visible == 'true' && verifyPermissionMenu('/control/topic/manage?method=add*','get')" @click="$router.push({path: '/admin/control/topic/manage/add', query:{ visible : 'true',page:($route.query.page != undefined ? $route.query.page:'')}})">添加话题</el-button>
            <el-button type="primary" plain v-if="state.visible == 'true'" @click="$router.push({path: '/admin/control/topic/list', query:{ visible : 'false'}})">回收站</el-button>
                ...
                <el-table-column label="操作" align="center" min-width="110">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/topic/manage?method=view*','get')" @click="store.setCacheNumber();$router.push({path: '/admin/control/topic/manage/view', query:{ visible : ($route.query.visible != undefined ? $route.query.visible:''),topicId : scope.row.id}})">查看</el-button>
