...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/comment/manage?method=deleteReply*','post')" @click="deleteReply($event,undefined)">批量删除</el-button>
        </div>
                ...
                <el-table-column label="操作" align="center" width="150">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/topic/manage?method=view*','get')" @click="store.setCacheNumber();$router.push({path: '/admin/control/topic/manage/view', query:{ topicId : scope.row.topicId, commentId:scope.row.commentId, replyId:scope.row.id}})">查看</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/comment/manage?method=deleteReply*','post')" @click="deleteReply($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    const queryReplyList = () => {
        ...
        proxy?.$axios({
            url: '/control/topic/allAuditReply',
            method: 'get',
            params: {
            ...
            if(response){
                ...
                if(result){
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		let replyList = pageView.records;
			    		if(replyList != null && replyList.length >0){
			    			state.tableData = replyList;
    ...
    const page = (page:number) => {
        ...
        router.push({
            path: '/admin/control/topic/allAuditReply', 
            query:{ page : page}
        });
    }
    ...
    const deleteReply = (event:any,row:any) => {
        ...
        .then(() => {
            ...
            for(let i=0; i<state.multipleSelection.length; i++){
                ...
                formData.append('replyId', rowData.id);
            }
