...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/thumbnail/manage?method=add*','get')" @click="$router.push({path: '/admin/control/thumbnail/manage/add'});">添加缩略图裁剪尺寸</el-button>
        </div>
                ...
                <el-table-column label="操作" align="center" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/thumbnail/manage?method=delete&*','post')" @click="deleteThumbnail($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    //查询缩略图列表
    const queryThumbnailList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/thumbnail/list',
            method: 'get',
            params: {
    ...
    //删除缩略图列表
    const deleteThumbnail = (event:any,row:any) => {
        //强制失去焦点
