...
<template>
    <div class="main">
        <div class="navbar">
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/thirdPartyLoginInterface/manage?method=add*','get')" @click="$router.push({path: '/admin/control/thirdPartyLoginInterface/manage/add'});">添加第三方登录接口</el-button>
        </div>
            ...
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                ...
                <el-table-column label="接口产品" align="center" >
                    <template #default="scope">
                        ...
                        <span v-if="scope.row.interfaceProduct == 50">其他开放平台</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center" width="200">
                    <template #default="scope">
                        <div class="button-group-wrapper">
                            <el-button-group>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/thirdPartyLoginInterface/manage?method=edit&*','get')" @click="$router.push({path: '/admin/control/thirdPartyLoginInterface/manage/edit', query:{ thirdPartyLoginInterfaceId : scope.row.id, page:state.currentpage}})">修改</el-button>
                                <el-button type="primary" v-if="verifyPermissionMenu('/control/thirdPartyLoginInterface/manage?method=delete*','post')" @click="deleteThirdPartyLoginInterface($event,scope.row)">删除</el-button>
                            </el-button-group>
                        </div>
                    </template>
...
<script lang="ts" setup>
    ...
    //查询第三方登录接口列表
    const queryThirdPartyLoginInterfaceList = () => {
        //清空内容
        state.tableData = []; 


        proxy?.$axios({
            url: '/control/thirdPartyLoginInterface/list',
            method: 'get',
            params: {
    ...
    //删除第三方登录接口
    const deleteThirdPartyLoginInterface = (event:any,row:any) => {
        //强制失去焦点
                ...
                if(response){
                    ...
                    if(result){
				    	...
				    	if(returnValue.code === 200){//成功
				    		...
				    		queryThirdPartyLoginInterfaceList();
				    	}else if(returnValue.code === 500){//错误
