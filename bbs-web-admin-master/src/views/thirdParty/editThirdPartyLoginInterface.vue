        ...
        <div class="data-form label-width-blank" >
            <el-form label-width="auto"  @submit.native.prevent>
                ...
                <el-form-item label="接口产品" >
                    <span v-if="state.interfaceProduct == 10">微信</span>
                    <span v-if="state.interfaceProduct == 50">其他开放平台</span>
                </el-form-item>
...
<script lang="ts" setup>
    ...
    //查询修改第三方登录接口
    const queryEditThirdPartyLoginInterface = () => {
        ...
        proxy?.$axios({
            ...
            params: {
                method : 'edit',
			    thirdPartyLoginInterfaceId :state.thirdPartyLoginInterfaceId
            },
                ...
                if(result){
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "thirdPartyLoginInterface"){
			    				let thirdPartyLoginInterface = mapData[key];
			    				
			    				state.name = thirdPartyLoginInterface.name;
			    				state.interfaceProduct = thirdPartyLoginInterface.interfaceProduct;
			    				state.enable = thirdPartyLoginInterface.enable;
			    				state.sort = thirdPartyLoginInterface.sort;
			    			}else if(key == "weChatConfig"){
			    				let weChatConfig = mapData[key];
			    				state.weixin_op_appID = weChatConfig.op_appID;
			    				state.weixin_op_appSecret = weChatConfig.op_appSecret;
			    				state.weixin_oa_appID = weChatConfig.oa_appID;
			    				state.weixin_oa_appSecret = weChatConfig.oa_appSecret;
			    			}else if(key == "otherConfig"){
			    				let otherConfig = mapData[key];
			    				state.other_appID = otherConfig.appID;
			    				state.other_appSecret = otherConfig.appSecret;
    ...
    const submitForm = () => {
        ...
        formData.append('thirdPartyLoginInterfaceId', state.thirdPartyLoginInterfaceId);
        ...
        proxy?.$axios({
            url: '/control/thirdPartyLoginInterface/manage?method=edit&a=a',//a=a参数的作用是仅增加连接符&
            method: 'post',
            data: formData,
            //showLoading: false,//是否显示加载图标
            loadingMask:false,// 是否显示遮罩层
    ...
    onMounted(() => {
        ...
        if(router.currentRoute.value.query.thirdPartyLoginInterfaceId != undefined && router.currentRoute.value.query.thirdPartyLoginInterfaceId != ''){
			state.thirdPartyLoginInterfaceId = router.currentRoute.value.query.thirdPartyLoginInterfaceId as string;
		}
		
		
		queryEditThirdPartyLoginInterface();
    }) 
