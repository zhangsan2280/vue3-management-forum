        ...
        <div class="data-form label-width-blank userModule" >
            <el-form label-width="auto"  @submit.native.prevent>
                ...
                <el-form-item label="备注"  :error="error.userDesc"  >
                    <el-input v-model.trim="state.userDesc" maxlength="150" :disabled="true" :clearable="true" show-word-limit></el-input>					
                </el-form-item>
                ...
                <el-form-item label="是否使用" :required="true" :error="error.enabled">
                    <el-radio-group v-model="state.enabled">
                        <el-radio :label="true" :disabled="true" >启用</el-radio>
                        <el-radio :label="false" :disabled="true" >停用</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="用户角色" >
                    <el-checkbox-group v-model="state.sysRolesId">
                        <el-checkbox :label="roles.id" :disabled="true" :checked="roles.selected" v-for="roles in state.rolesList">{{roles.name}}</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
...
<script lang="ts" setup>
 ...
 const settingAvatarUI = () => {
    ...
    const submitForm = () => {
        ...
        proxy?.$axios({
            url: '/control/staff/manage?method=editSelfInfo',
            method: 'post',
            data: formData,
            timeout: 0,// 定义请求超时时间
            //showLoading: false,//是否显示加载图标
